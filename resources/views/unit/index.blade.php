<x-app-layout>
    <div x-data="unitManager" x-init="init()" class="max-w-7xl mx-auto p-6">

        <!-- HEADER -->
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Unit</h1>
            ...
            <div class="p-6 border-b border-gray-100">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div class="relative w-full md:w-72">
                        <input type="text" x-model="search" @input="searchData()" placeholder="Cari unit..."
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <template x-for="(unit, index) in units" :key="unit.id">
                            <tr class="hover:bg-indigo-50/50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4">
                                    <span class="font-medium text-gray-800" x-text="unit.name"></span>
                                </td>
                                <td class="px-6 py-4 text-gray-600" x-text="unit.description || '-'"></td>
        ...
        <x-modal name="create-unit" maxWidth="md">
            <div class="p-6">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <button x-on:click="$dispatch('close-modal','create-unit')"
                ...
                <form @submit.prevent="storeUnit()">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea x-model="form.description" rows="3"
                            class="w-full border border-gray-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-indigo-200 focus:border-indigo-500 transition-all"></textarea>
                    </div>
        ...
        <!-- MODAL EDIT -->
        <x-modal name="edit-unit" maxWidth="md">
            <div class="p-6">
                <div class="flex justify-between items-center mb-6">
                    <h2 class="text-lg font-semibold text-gray-800">Edit Unit</h2>
                    <button x-on:click="$dispatch('close-modal','edit-unit')"
                        class="text-gray-400 hover:text-gray-600">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M6 18L18 6M6 6l12 12" />
                ...
                <form @submit.prevent="updateUnit()">
                    ...
                    <div class="flex justify-end gap-3 mt-6">
                        <button type="button" x-on:click="$dispatch('close-modal','edit-unit')"
                            class="px-4 py-2 border border-gray-200 rounded-xl hover:bg-gray-50 text-gray-700 transition-colors">
                            Batal
                        </button>
                        <button type="submit"
        ...
        <x-modal name="confirm-delete-unit" maxWidth="sm">
            <div class="p-6 text-center">
                ...
                <h2 class="text-lg font-semibold text-gray-800 mb-2">Hapus Unit?</h2>
                <p class="text-sm text-gray-500 mb-6">
                    Data yang dihapus tidak bisa dikembalikan. Yakin ingin menghapus <strong
                        x-text="deleteName"></strong>?
                </p>
                <div class="flex justify-center gap-3">
                    <button @click="$dispatch('close-modal','confirm-delete-unit')"
                        class="px-4 py-2 border border-gray-200 rounded-xl hover:bg-gray-50 text-gray-700 transition-colors">
                        Batal
                    </button>
        ...
        function unitManager() {
            return {
                ...
                storeUnit() {
                    ...
                    fetch('/units', {
                        ...
                        .then(data => {
                            ...
                            this.fetchData()
                            this.showToast('success', 'Unit berhasil ditambahkan!')
                            this.form = {
                                id: null,
                                name: '',
                                description: ''
                ...
                updateUnit() {
                    ...
                    fetch(`/units/${this.form.id}`, {
                        ...
                        .then(data => {
                            ...
                            this.fetchData()
                            this.showToast('success', 'Unit berhasil diupdate!')
                        })
                        .catch(err => {
                            this.saving = false
                            if (err.errors) {
                ...
                deleteUnit(id) {
                    ...
                    this.deleteId = id
                    this.deleteName = unit ? unit.name : ''
                    window.dispatchEvent(new CustomEvent('open-modal', {
                        detail: 'confirm-delete-unit'
                    }))
                },
                ...
                confirmDelete() {
                    ...
                    fetch(`/units/${this.deleteId}`, {
                        ...
                        .then(data => {
                            ...
                            this.fetchData()
                            this.showToast('success', 'Unit berhasil dihapus!')
                            this.deleteId = null
                            this.deleteName = ''
                        })
                        .catch(err => {
